...
import (
	...
	"github.com/FiiLabs/block_explorer/libs/msgparser"
...
func ParseBlockAndTxs(b int64, client *pool.Client) (*models.Block, []*models.Tx, []*models.NFTCls, []*models.NFT, error) {
	var (
	...
	if v, err := client.Block(ctx, &b); err != nil {
		...
		if v2, err := client.Block(ctx, &b); err != nil {
			return &blockDoc, nil, nil, nil, utils.ConvertErr(b, "", "ParseBlock", err)
		} else {
			block = v2
		}
	} else {
	...
	if blockDoc.Txn <= 0 {
		return &blockDoc, nil, nil, nil, nil
	}
	...
	if err != nil {
		...
		blockResults, err = client.BlockResults(context.Background(), &b)
		if err != nil {
			return &blockDoc, nil, nil, nil, utils.ConvertErr(b, "", "ParseBlockResult", err)
		}
	}

	if len(block.Block.Txs) != len(blockResults.TxsResults) {
		return nil, nil, nil, nil, utils.ConvertErr(b, "", "block.Txs length not equal blockResult", nil)
	}

	txDocs := make([]*models.Tx, 0, len(block.Block.Txs))
	nftClses := make([]*models.NFTCls, 0, len(block.Block.Txs))
	nfts := make([]*models.NFT, 0, len(block.Block.Txs))
	if len(block.Block.Txs) > 0 {
		for i, v := range block.Block.Txs {
			txResult := blockResults.TxsResults[i]
			txDoc, nftCls, nftx, err := parseTx(v, txResult, block.Block, i)
			if err != nil {
				return &blockDoc, txDocs, nftClses, nfts, err
			}
	...
	return &blockDoc, txDocs, nftClses, nfts, nil
}

func parseTx(txBytes types.Tx, txResult *types2.ResponseDeliverTx, block *types.Block, index int) (models.Tx, models.NFTCls, models.NFT, error) {
	var (
		docTx          models.Tx
		nftCls         models.NFTCls
		nftx            models.NFT
	...
	for i, v := range msgs {
		...
		case MsgTypeIssueDenom:
			...
			// get denom_id from events then set to msg, because this msg hasn't denom_id
			denomId := ParseAttrValueFromEvents(docTx.EventsNew[i].Events, EventTypeIssueDenom, AttrKeyDenomId)
			msgDocInfo.DocTxMsg.Msg.(*nft.DocMsgIssueDenom).Id = denomId
			if nftIssueDenomMsg, ok := msgDocInfo.DocTxMsg.Msg.(*nft.DocMsgIssueDenom); ok {
				nftCls.Id                = nftIssueDenomMsg.Id
				nftCls.ClsName           = nftIssueDenomMsg.Name
				nftCls.Schema            = nftIssueDenomMsg.Schema
				nftCls.Sender            = nftIssueDenomMsg.Sender
				nftCls.Symbol            = nftIssueDenomMsg.Symbol
				nftCls.MintRestricted    = nftIssueDenomMsg.MintRestricted
				nftCls.UpdateRestricted  = nftIssueDenomMsg.UpdateRestricted
				nftCls.Description       = nftIssueDenomMsg.Description
				nftCls.Uri               = nftIssueDenomMsg.Uri
				nftCls.UriHash           = nftIssueDenomMsg.UriHash
				nftCls.Data              = nftIssueDenomMsg.Data
				nftCls.Time              = docTx.Time
				nftCls.SetAction(models.TxActionNFTClsMint)
			}
		case MsgTypeTransferDenom:
			if docTx.Status == constant.TxStatusFail {
				break
			}

			// get nft_id from events then set to msg, because this msg hasn't nft_id
			nftclsId := ParseAttrValueFromEvents(docTx.EventsNew[i].Events, EventTypeTransferNFTCls, AttrKeyDenomId)
			msgDocInfo.DocTxMsg.Msg.(*nft.DocMsgTransferDenom).Id = nftclsId
			if nftClsTransferMsg, ok := msgDocInfo.DocTxMsg.Msg.(*nft.DocMsgTransferDenom); ok {
				nftCls.Id                = nftClsTransferMsg.Id
				tid,err := nftCls.GetNFTClsById(nftCls.Id)
				if err == nil {
					nftCls.SetID(tid)
				}
				nftCls.Id                = nftClsTransferMsg.Id
				nftCls.Sender            = nftClsTransferMsg.Recipient
				nftCls.Time              = docTx.Time
				nftCls.SetAction(models.TxActionNFTClsTransfer)
			}
		case MsgTypeNFTMint:
			if docTx.Status == constant.TxStatusFail {
				break
			...
			// get nft_id from events then set to msg, because this msg hasn't nft_id
			nftId := ParseAttrValueFromEvents(docTx.EventsNew[i].Events, EventTypeMintNFT, AttrKeyNFTId)
			msgDocInfo.DocTxMsg.Msg.(*nft.DocMsgNFTMint).Id = nftId
			if nftMintMsg, ok := msgDocInfo.DocTxMsg.Msg.(*nft.DocMsgNFTMint); ok {
				nftx.Id                = nftMintMsg.Id
				nftx.NTFName           = nftMintMsg.Name
				nftx.DenomId           = nftMintMsg.Denom
				nftx.URI               = nftMintMsg.URI
				nftx.Data              = nftMintMsg.Data
				nftx.Sender            = nftMintMsg.Sender
	...
	if len(_filterMap) > 0 && !includeCfgType {
		logger.Warn("skip tx for no include setting types",
			...
			logger.String("txhash", txHash),
			logger.Int64("height", block.Height))
		return models.Tx{}, models.NFTCls{}, models.NFT{}, nil
	}

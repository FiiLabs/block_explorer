...
func (s *syncTaskService) TakeOverTaskAndExecute(task models.SyncTask, client *pool.Client, healthCheckQuit chan bool, blockNumPerWorkerHandle int64) {
	...
	for isValid {
		...
		blockDoc, txDocs, NFTClses, NFTs, err := handlers.ParseBlockAndTxs(inProcessBlock, client)
		if err != nil {
		...
		if workerUnchanged {
			...
			err := saveDocsWithTxn(blockDoc, txDocs, NFTClses, NFTs, &taskDoc)
			if err != nil {
...
func saveDocsWithTxn(blockDoc *models.Block, txDocs []*models.Tx, nftClses []*models.NFTCls, nfts []*models.NFT, taskDoc *models.SyncTask) error {
	...
	_, err := models.GetClient().DoTransaction(ctx, func(sessCtx context.Context) (interface{}, error) {
		...
		nftClsCli := models.GetClient().Database(models.GetDbConf().Database).Collection(models.NFTCls{}.Name())
		nftCli := models.GetClient().Database(models.GetDbConf().Database).Collection(models.NFT{}.Name())
